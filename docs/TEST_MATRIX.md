...
| TM-004 | Enregistrement d'un brouillon dans le Wizard | Marchand connecté | Saisir un jeu incomplet puis enregistrer le brouillon | Le brouillon est enregistré, l'écran de confirmation propose prévisualisation, QR code et affiche, puis le jeu apparaît immédiatement dans Campagnes | `apps/web-app/e2e/campaign-deletion.spec.ts` | Partiel : confirmation et suppression automatisées ; contrôle métier du brouillon à compléter |
...
| TM-020 | Suppression d’un jeu marchand | Compte `e2e@okado.app`, brouillon de test | Ouvrir les actions du jeu, confirmer sa suppression | La pop-in est explicite et fermable ; après confirmation, le jeu disparaît immédiatement de Campagnes | `apps/web-app/e2e/campaign-deletion.spec.ts` | Automatisé |

...
function enrichLeadRowsWithEmailDeliveries(
  ...
  return leads.map((lead) => {
    ...
    // Un lead sans lot ne peut pas Ãªtre Â« Ã€ retirer Â». Cette normalisation
    // protÃ¨ge lâ€™interface contre les anciennes donnÃ©es orphelines (par
    // exemple aprÃ¨s la suppression dâ€™un lot) et les affiche comme perdues.
    const normalizedLead =
      !lead.prizeId && lead.status === "claimed" ? { ...lead, status: "lost" as const } : lead;

    retur…18818 tokens truncated…(data as PrizeRow);
}
...
export async function resetPrizeStockInSupabase(prizeId: string) {
  ...
  if (protectedResetError || !protectedReset) {
    throw new Error(protectedResetError?.message || "Le stock n'a pas pu Ãªtre rÃ©initialisÃ©.");
  }
  ...
  if (error || !data) {
    throw new Error("Le stock n'a pas pu Ãªtre rÃ©initialisÃ©");
  }
...
export async function getSupabaseLeadRewardEmailHistory(leadId: string, merchantId: string) {
  ...
  if (eventsError) {
    throw new Error(`Lecture des Ã©vÃ©nements e-mail impossible: ${eventsError.message}`);
  }
...
export async function getSupabaseRewardEmailResendPayload(leadId: string, merchant: Merchant) {
  ...
  if (!lead.prizeId || !lead.redemptionCode) {
    throw new Error("Aucun lot gagnant Ã  renvoyer");
  }

  if (lead.status !== "claimed") {
    throw new Error("Seuls les gains disponibles peuvent Ãªtre renvoyÃ©s.");
  }
  if (lead.rewardExpiresAt && new Date(lead.rewardExpiresAt).getTime() < Date.now()) {
    throw new Error("Ce gain est expirÃ©. RÃ©initialisez-le avant de renvoyer lâ€™e-mail.");
  }
  ...
  if (delivery?.status === "queued") {
    throw new Error("Un e-mail est dÃ©jÃ  en cours d'envoi.");
  }
...
export async function markRewardEmailSentInSupabase(
  ...
  if (error) {
    throw new Error(`Mise Ã  jour de l'email envoyÃ© impossible: ${error.message}`);
  }
}
...
export async function markRewardEmailFailedInSupabase(deliveryId: string, errorMessage: string) {
  ...
  if (retryError) {
    throw new Error(`Mise Ã  jour de l'e-mail en Ã©chec impossible: ${retryError.message}`);
  }
  return;
  ...
  if (error) {
    throw new Error(`Mise Ã  jour de l'email en Ã©chec impossible: ${error.message}`);
  } */
}
...
export async function syncRewardEmailWebhookInSupabase(event: WebhookEventPayload) {
  ...
  if (resendEmailId) {
    ...
    if (deliveryId && event.type === "email.failed") {
      const retryReason = event.data.failed.reason || "Ã‰chec Resend";
...
export async function getSupabaseMerchantSupportOverview(
  ...
  if (failedEmailResult.error) {
    throw new Error(`Lecture des e-mails en Ã©chec impossible: ${failedEmailResult.error.message}`);
  }

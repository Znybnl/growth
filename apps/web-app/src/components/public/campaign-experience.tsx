...
export function CampaignExperience({
  ...
  const logoTextSizePx = Math.round(
    campaignLogoTextSizePx(logoSizePercent, campaign.gameType) * (isBeautyTemplate || isRoseInstitutTemplate ? 0.9 : 1),
  );
  ...
  const backgroundStyle =
    ...
    campaign.presentation.background.imageUrl
      ...
      : isScratchVaultTemplate
        ...
        : isScratchSunburstTemplate
          ? `repeating-conic-gradient(from -18deg at 50% -2%, ${withHexAlpha(primaryColor, "52")} 0deg 12deg, transparent 12deg 24deg), linear-gradient(180deg, #fff4bf99 0%, #ffdc5880 68%, #fff0c599 100%)`
        : isCosmicTemplate
  ...
  return (
    ...
    >
      {isBeautyTemplate && !(campaign.presentation.background.mode === "image" && campaign.presentation.background.imageUrl) ? (
      ...
      <div className={`relative z-10 mx-auto flex ${isBeautyTemplate ? "min-h-[calc(100dvh-44px)]" : isPreview ? "h-[calc(100dvh-44px)] min-h-[560px]" : "h-screen"} w-full flex-col ${isBeautyTemplate ? "overflow-visible pb-8" : "overflow-hidden pb-0"} px-4 sm:px-6 ${pageTopPaddingClass}`}>
        ...
        <div className={headingAlignmentClass}>
          ...
          /> : <h1
            className={`${headingFontClass} ${isBeautyTemplate ? "line-clamp-3 okado-beauty-heading" : isRoseInstitutTemplate ? "line-clamp-5 leading-[1]" : "line-clamp-3 leading-[1]"} whitespace-pre-line text-[#151826]`}

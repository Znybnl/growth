            ...
            >
              ...
              >
                ...
                <div
                  ...
                  style={{ borderColor: theme.id === "beauty-rose" ? "#e7aebb" : undefined, background: theme.id === "beauty-rose" ? `repeating-conic-gradient(from -22deg, ${theme.secondary} 0deg 44.5deg, rgba(185,95,117,.35) 44.5deg 45deg, #f3dfe3 45deg 89.5deg, rgba(185,95,117,.35) 89.5deg 90deg, #e9bbc5 90deg 134.5deg, rgba(185,95,117,.35) 134.5deg 135deg)` : `repeating-conic-gradient(from -22deg, ${theme.primary} 0deg 45deg, ${theme.secondary} 45deg 90deg)`, boxShadow: theme.id === "beauty-rose" ? "0 10px 30px rgba(90,45,60,.1), 0 0 0 4px #fffdfc, 0 0 0 5px #f3d9df" : undefined }}
                >
                  {theme.id === "beauty-rose" ? <svg className="absolute left-1/2 top-[-3px] h-7 w-5 -translate-x-1/2 overflow-visible" viewBox="0 0 44 52" aria-hidden="true"><path d="M22 2C33 2 40 10 38 20C36 30 27 40 22 49C17 40 8 30 6 20C4 10 11 2 22 2Z" fill="#b95f75" stroke="#fffdfc" strokeWidth="2.5" /></svg> : null}
                  <span className={`absolute left-1/2 top-1/2 flex ${theme.id === "beauty-rose" ? "h-14 w-14" : "h-12 w-12"} -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-full text-[8px] font-bold ${theme.id === "beauty-rose" ? "border-2 border-[#e7aebb] text-[#b95f75] shadow-[0_2px_7px_rgba(90,45,60,.12)]" : "border-[3px] border-white text-white"}`} style={{ background: theme.id === "beauty-rose" ? "#fffdfc" : theme.primary }}>{theme.id === "beauty-rose" ? <RoseFlowerMark className="h-6 w-6" /> : <Pointer aria-hidden="true" className="h-4 w-4" />}JOUER</span>
                </div>
              </div>

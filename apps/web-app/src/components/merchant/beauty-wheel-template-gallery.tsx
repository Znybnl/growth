            ...
            >
              ...
              >
                ...
                <div
                  ...
                  style={{ borderColor: theme.id === "beauty-rose" ? "#e7aebb" : undefined, background: theme.id === "beauty-rose" ? "repeating-conic-gradient(from -22deg, #fffdfc 0deg 44.5deg, rgba(185,95,117,.35) 44.5deg 45deg, #f3cdd5 45deg 89.5deg, rgba(185,95,117,.35) 89.5deg 90deg, #e7aebb 90deg 134.5deg, rgba(185,95,117,.35) 134.5deg 135deg)" : `repeating-conic-gradient(from -22deg, ${theme.primary} 0deg 45deg, ${theme.secondary} 45deg 90deg)`, boxShadow: theme.id === "beauty-rose" ? "0 12px 30px rgba(90,45,60,.12), 0 0 0 4px #fffdfc, 0 0 0 5px #e7aebb" : undefined }}
                >
                  {theme.id === "beauty-rose" ? <svg className="absolute left-1/2 top-[-3px] h-6 w-6 -translate-x-1/2 overflow-visible" viewBox="0 0 48 44" aria-hidden="true"><path d="M24 2C37 2 45 11 41 22C38 29 29 38 24 42C19 38 10 29 7 22C3 11 11 2 24 2Z" fill="#b95f75" stroke="#fffdfc" strokeWidth="2" /></svg> : null}
                  <span className={`absolute left-1/2 top-1/2 flex ${theme.id === "beauty-rose" ? "h-[58px] w-[58px]" : "h-12 w-12"} -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-full ${theme.id === "beauty-rose" ? "text-[9px]" : "text-[8px]"} font-bold ${theme.id === "beauty-rose" ? "border-2 border-[#e7aebb] text-[#b95f75] shadow-[0_2px_7px_rgba(90,45,60,.12)]" : "border-[3px] border-white text-white"}`} style={{ background: theme.id === "beauty-rose" ? "#fffdfc" : theme.primary }}>{theme.id === "beauty-rose" ? <RoseFlowerMark className="h-5 w-5" /> : <Pointer aria-hidden="true" className="h-4 w-4" />}JOUER</span>
                </div>
              </div>

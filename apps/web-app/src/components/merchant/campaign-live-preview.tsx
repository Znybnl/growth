      ...
      <div className={`mx-auto w-full overflow-hidden border border-[#ced7e6] shadow-[0_30px_70px_rgba(18,24,39,0.18)] ${previewFrameClass}`} style={{ ...preview.backgroundStyle, ...(compact ? { containerType: "inline-size" } : {}) }}>
        {showStandardHeader ? (
          <>
            ...
            {preview.gameType === "wheel" && preview.wheelSubtitle.trim() ? <p className={`${preview.headingAlignmentClass} mt-3 px-4 text-sm font-medium leading-6 sm:text-base`} style={{ color: previewHeadingTextColor }}>{preview.wheelSubtitle}</p> : null}
          </>
        ) : null}

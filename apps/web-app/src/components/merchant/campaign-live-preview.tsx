      ...
      <div className={`mx-auto w-full ${preview.gamePageTemplateId === "beauty-rose" && compact ? "overflow-x-hidden overflow-y-auto" : "overflow-hidden"} border border-[#ced7e6] shadow-[0_30px_70px_rgba(18,24,39,0.18)] ${previewFrameClass} ${preview.gamePageTemplateId === "beauty-rose" ? "okado-rose-powder-surface relative" : ""}`} style={{ ...preview.backgroundStyle, ...(compact ? { containerType: "inline-size" } : {}) }}>
        ...
        {showStandardHeader ? (
          <>
            ...
            {preview.logoMode === "text" ? <div className={`flex ${preview.logoAlignmentClass}`}><div style={{ marginBottom: `${scalePreviewValue(preview.logoBottomSpacingPx)}px` }}><BrandMark logoText={preview.logoText} size="lg" variant="transparent" imageWidthPx={scalePreviewValue(preview.logoWidthPx)} textSizePx={scalePreviewValue(preview.logoTextSizePx) * (isRosePowderTemplate ? 0.9 : 1)} textColor={preview.logoTextColor} textClassName="text-2xl" textFontWeight={isRosePowderTemplate ? 600 : undefined} /></div></div> : null}
            {preview.gameType === "scratch" && preview.logoMode === "none" ? <div className={`flex ${preview.logoAlignmentClass}`}><div style={{ marginBottom: `${scalePreviewValue(preview.logoBottomSpacingPx)}px` }}><BrandMark logoText={preview.logoText || merchant.companyName} size="lg" variant="transparent" imageWidthPx={scalePreviewValue(preview.logoWidthPx)} textSizePx={scalePreviewValue(preview.logoTextSizePx) * (isRosePowderTemplate ? 0.9 : 1)} textColor={preview.logoTextColor} textClassName="text-2xl" textFontWeight={isRosePowderTemplate ? 600 : undefined} /></div></div> : null}

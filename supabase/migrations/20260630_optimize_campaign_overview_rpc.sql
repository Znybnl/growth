...
create index if not exists idx_campaign_actions_campaign_position
  on public.campaign_actions (campaign_id, position);

create index if not exists idx_prizes_campaign
  on public.prizes (campaign_id);
...
create index if not exists idx_leads_campaign_status
  on public.leads (campaign_id, status);

create index if not exists idx_leads_campaign_prize
  on public.leads (campaign_id, prize_id);

create index if not exists idx_leads_campaign_redemption_code
  on public.leads (campaign_id, redemption_code);
...
create index if not exists idx_reward_email_deliveries_campaign_lead
  on public.reward_email_deliveries (campaign_id, lead_id);
...
as $$
  ...
  lead_stats as (
    ...
    from public.leads l
    join merchant_campaigns mc on mc.id = l.campaign_id
  ...
  event_stats as (
    ...
    join merchant_campaigns mc on mc.id = e.campaign_id

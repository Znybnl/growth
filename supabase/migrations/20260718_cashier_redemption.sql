...
alter table public.leads
  ...
  add column if not exists redeemed_by_user_id text,
  add column if not exists purchase_verified boolean not null default false;

create index if not exists leads_redemption_code_lower_idx
  on public.leads (lower(redemption_code));

create table if not exists public.cashier_redemption_audits (
  id text primary key,
  ...
  status text not null check (status in ('redeemed', 'rejected')),
...
create unique index if not exists cashier_redemption_audits_idempotency_idx
  on public.cashier_redemption_audits (merchant_id, idempotency_key)
  where idempotency_key is not null;

create index if not exists cashier_redemption_audits_merchant_created_idx
  on public.cashier_redemption_audits (merchant_id, created_at desc);
...
begin
  ...
  select * into v_campaign from public.campaigns where id = v_lead.campaign_id;
...
revoke all on table public.cashier_redemption_audits from public, anon, authenticated;
